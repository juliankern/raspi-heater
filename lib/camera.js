...
Camera.prototype.handleSnapshotRequest = function(request, callback) {
    ...
    var snapshot = fs.readFileSync(__dirname + '/../node_modules/hap-nodejs/lib/res/snapshot.jpg');
    ...
    if (sessionID) {
        ...
        if (requestType == "start") {
            ...
            if (sessionInfo) {
                ...
                let ffmpegCommand = 'ffmpeg -re -f avfoundation -r 29.970000 -i 0:0 -threads 0 -vcodec libx264 -an -pix_fmt yuv420p -r '+ fps
                    + ' -f rawvideo -tune zerolatency -vf scale='+ width +':'+ height +' -b:v '+ bitrate +'k -bufsize '+ bitrate 
                    + 'k -payload_type 99 -ssrc 1 -f rtp -srtp_out_suite AES_CM_128_HMAC_SHA1_80 -srtp_out_params '+videoKey.toString('base64')
                    + ' srtp://'+targetAddress+':'+targetVideoPort+'?rtcpport='+targetVideoPort+'&localrtcpport='+targetVideoPort+'&pkt_size=1378';
                    
                let ffmpeg = spawn('./bin/stream.sh', [ width, height, fps, bitrate, videoKey.toString('base64'), targetAddress, targetVideoPort ], {env: process.env});
